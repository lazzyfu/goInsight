...
goInsight是一个集成数据库工单和数据检索的平台，提供语法审核、工单工作流、数据库表查询能力，旨在规范流程、减少风险、解放DBA生产力。
...
- **组织管理** - 支持用户组织层级管理，可以为DB实例绑定组织，实现用户只访问当前组织的数据库，支持用户角色管理。
- **其他功能**
  - 支持双因素身份认证登录、支持日志审计
...
## 限制 
  - **工单** - 目前仅支持MySQL5.6+、TiDB数据库，支持提交DML/DDL语句，单个工单最大限制提交2048条SQL语句
  - **查询** - 目前仅支持MySQL、TiDB、ClickHouse数据库，默认返回100条记录，仅支持只读操作，变更请使用工单。
