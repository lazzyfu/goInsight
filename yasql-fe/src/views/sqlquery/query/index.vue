  ...
  >
    <a-row>
      <div class="query-header">
        <span slot="title">
          ...
          <span
            >字符集：
            <el-select placeholder="选择字符集" style="width: 15%" size="mini" v-model="selectedCharacter">
              <el-option v-for="item in characterSet" :key="item.key" :label="item.key" :value="item.value"></el-option>
            </el-select>
          </span>
          <el-divider direction="vertical"></el-divider>
          <span
            ...
            <el-select
              ...
              @change="setCodeTheme"
            >
              <el-option v-for="item in themes" :key="item.key" :label="item.key" :value="item.value"></el-option>
            </el-select>
          </span>
        </span>
      </div>
    ...
    <div class="box" ref="box">
      <div class="left">
        <a-row>
            ...
            <a-form layout="inline" :form="form" @keyup.enter.native="handleSearch">
              <a-form-item>
                ...
                >
                  ...
                  <a-select-option v-for="s in databases" :key="s.key" :value="s.key">{{ s.title }}</a-select-option>
                </a-select>
              </a-form-item>
            </a-form>
          </div>
          ...
          <div v-loading="treeLoading" element-loading-text="玩命加载中..." element-loading-spinner="el-icon-loading">
          ...
          <v-contextmenu ref="contextmenu">
            <v-contextmenu-item @click="viewTableStruc">查看表结构</v-contextmenu-item>
            <v-contextmenu-item @click="viewTableBase">查看表信息</v-contextmenu-item>
          </v-contextmenu>
        </a-row>
      </div>
          ...
          <div id="cmHeight">
            <div style="margin-bottom: 6px; margin-top: 4px">
              <el-button-group style="margin-left: 2px; margin-right: 2px">
                <el-tooltip class="item" effect="dark" content="每次仅允许执行一条SQL" placement="top">
                  <el-button size="mini" @click="executeSQL()" icon="el-icon-video-play">执行SQL</el-button>
                </el-tooltip>

                <el-tooltip class="item" effect="dark" content="格式化SQL" placement="top">
                  <el-button size="mini" @click="formatSQL()" icon="el-icon-edit">格式化</el-button>
                </el-tooltip>
                <el-tooltip class="item" effect="dark" content="查看执行过的历史SQL" placement="top">
                  <el-button size="mini" @click="getHistorySQL()" icon="el-icon-document">我的SQL</el-button>
                </el-tooltip>
                <el-button size="mini" @click="loadDBDict()" icon="el-icon-notebook-1">数据字典</el-button>
                <el-button size="mini" @click="dialogVisible = true" icon="el-icon-document">使用技巧</el-button>
              </el-button-group>
            </div>
            <div>
              <codemirror ref="myCm1" v-model="code1" :options="cmOptions1" @ready="onCmReady1"></codemirror>
            </div>

            <div style="margin-top: 4px">
              <codemirror ref="myCm2" v-model="code2" :options="cmOptions2" @ready="onCmReady2"></codemirror>
            </div>
          </div>
        </a-row>
      </div>
    ...
    <template>
      <el-dialog title="使用小技巧" :visible.sync="dialogVisible">
        <p>* 默认返回100条记录，LIMIT N最大返回2000条记录。单条SQL查询超时为600秒。已开启SQL审计</p>
        <p>* 支持左右拉伸，点击中间蓝色部分即可实现左右拉伸</p>
        <p>* 支持SQL输入框上下拉伸（鼠标置于输入框的右下角进行拖动）</p>
        <p>* 如果SQL没有LIMIT，将会被自动加上LIMIT N</p>
        <p>* 支持LIMIT offset, row_count和LIMIT row_count OFFSET offset，row_count超过限制，将被重写</p>
    ...
    <!-- 数据字典 -->
    <template>
      <drawerDBDict ref="drawerDBDictChild" :visible="DBDictVisible" @close="DBDictVisible = false"></drawerDBDict>
    </template>
  </a-card>
</template>
      ...
      setTimeout(() => {
        getQueryTree({ key: value })
          .then((response) => {
            ...
            // 加载表列自动提示
            this.codemirror1.setOption('hintOptions', response.data.tab_completion)
            // 当库为空
            if (this.treeData.length === 0) {
              this.$notify.warning({
                title: '警告',
                message: '未从当前库' + value.split('___')[1] + '找到表，请确认库不为空',
              })
            }
          })
          .catch((err) => {
