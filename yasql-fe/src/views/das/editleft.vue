    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline" :form="form" @keyup.enter.native="handleSearch">
        <a-form-item>
            ...
            >
              ...
              <span v-else-if="s.db_type.toLowerCase() == 'tidb'">
                ...
                >
                  ...
                  ></path>
                  <path
                    d="M549.16 766.44l96.75-46.03V323.08l-96.75 51.66z"
                    fill="#d81e06"
                    p-id="5593"
                  ></path>
                </svg>
          ...
          <el-tooltip content="动态刷新库名" placement="top" effect="light">
            <el-button
              type="text"
              style="width: 10%; font-size: 18px"
        ...
        <div class="block">
          ...
          >
            <span class="custom-tree-node" slot-scope="{ node, data }">
              ...
              <span>
                <span v-if="node.label.split('#').length === 2">
                  <el-tooltip
                    v-if="node.label.split('#')[1] === 'allow'"
                    content="您有该表的访问权限 "
                  ...
                  >
                    <i
                      ...
                      style="color: #67c23a; margin-left: 2px"
                    >
                    </i>
                  </el-tooltip>
                  ...
                  >
                    <i
                      ...
                      style="color: #f56c6c; margin-left: 2px"
                    >
                    </i>
                  </el-tooltip>
                </span>
...
<script>
import { getSchemasApi, getTablesApi, getGrantsApi } from '@/api/das';
import { getClusterInstances } from '@/api/cmdb_mixin';
import drawerTableInfo from './tableinfo';
...
export default {
  ...
  methods: {
    ...
    async getGrants(params) {
      let data = [];
      await getGrantsApi(params).then((response) => {
        data = response.data;
      });
      return data;
    },
    ...
    renderTree(grants, data) {
      ...
      let checkTableRule = function (grants, table) {
        // 先检查黑名单表
        for (const [i, v] of grants.blacklist.entries()) {
          if (v['table'] === table) {
            return false;
          }
        }
      ...
      data.forEach(function (row) {
        ...
        // 表节点
        var rule =
          checkTableRule(grants, row.table_name) === true ? 'allow' : 'deny';
        tmpTreeData.push({
          label: `${row.table_name}#${rule}`,
    ...
    rightClick(MouseEvent, object, Node, element) {
      ...
      if (Node.key.split('#').length === 2) {
        const postition = {
          ...
          left: MouseEvent.pageX + 25,
        };
        let vals = Node.key.split('#');
        this.selectedKeys = {
          schema: vals[0],
          table: vals[1],
        };
    ...
    viewTableStruc() {
      ...
      this.$refs.drawerTableInfoChild.fetchData(params);
    },
    // 查看表元信息
    viewTableBase() {
      this.tableInfoVisible = true;
    ...
    // 从CMDB跳转过来
    loadSelectSchemaFromCMDB() {
      if (this.$route.query.hasOwnProperty('clusterId')) {
        getSchemasApi().then((response) => {
          if (response.code === '0001') {
            this.$notify.error({
              title: '加载失败',
          ...
          getClusterInstances(params).then((response) => {
            response.data.forEach((item) => {
              let component = item.component.toLowerCase();
              if (component === 'mysql' || component === 'clickhouse') {
                item.port.forEach((p) => {
                  if (p.type_c === 0) {
                    this.service_name.push([item.ip_addr, p.port].join(':'));
                  }
                });
              }
              if (component === 'tidb') {
                item.port.forEach((p) => {
                  if (p.type_c === 0 && p.port === 4000) {
                    this.service_name.push([item.ip_addr, p.port].join(':'));
                  }
                });
              }
            ...
            for (let hostname of this.service_name) {
              // hostname = 127.0.0.1:3306
              for (let item of this.schemas) {
                if (
                  [item.hostname, item.port].join(':') === hostname &&
                  item.schema === this.$route.query.schema
                ) {
                  this.getTables(
                    `${item.instance_id}#${item.schema}#${item.db_type}`
                  );
                  this.bindTitle = `${item.remark}:${item.schema}`;
                  havePrivilege = true;
                  return;
                }
              }
            ...
            if (!havePrivilege) {
              this.$notify.warning({
                title: 'WARNING',
                message: '权限不足, 已自动为您跳转到权限申请页面',
              });
              this.$router.push({
                path: '/dbms/other-orders/newquery',
