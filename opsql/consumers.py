...
class OrderMsgConsumer(WebsocketConsumer):
    received_value = None

    def connect(self):
        # 创建channels group， 命名为：用户名，并使用channel_layer写入到redis
        self.received_value = self.scope['url_route']['kwargs']['stream']
    ...
    def user_message(self, event):
        # websocket消费
        # self.received_value 用于前端页面的消息定位
        self.send(text_data=json.dumps({'tag': self.received_value, 'data': event["text"]}))
