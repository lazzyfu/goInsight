...
func NewTiDBCnx(cfg *base.DBConfig) (*sql.DB, error) {
...
func DaoTiDBQuery(db *sql.DB, query string) (*[]map[string]interface{}, error) {
...
func DaoTiDBExecute(db *sql.DB, sql string, ch chan<- int64) (int64, error) {
...
// 获取MySQL类数据库的连接ID
func DaoGetTiDBConnectionID(db *sql.DB) (connectionID int64, err error) {
...
func DaoTiDBGetProcesslist(dbconfig *base.DBConfig, order_id string, connection_id int64, ch <-chan int64) {
	// Create a new database connection
	db, err := NewTiDBCnx(dbconfig)
	if err != nil {
