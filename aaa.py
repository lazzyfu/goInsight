...
class ReadRemoteBinlog(object):
    ...
    def _generate_rollback_sql(self, rows):
        ...
        for row in rows:
            ...
            primary_key = format_row.get('primary_key')
            sql = ''

            if type == 'INSERT':
                where = '='.join((primary_key, str(row['values'].get(primary_key))))
                if not primary_key:
                ...
                where = '='.join((primary_key, str(row['after'].get(primary_key))))
                if not primary_key:
...
dd = ReadRemoteBinlog(binlog_file='mysql-bin.000013',
                      start_pos=2347530,
                      end_pos=2576761,
                      host='10.10.1.202',
                      port=3306,
