...
import (
	...
	"fmt"
	"goInsight/global"
	"goInsight/internal/app/das/forms"
	"goInsight/internal/app/das/models"
	"goInsight/internal/app/das/parser"
...
type ExecuteMySQLQueryService struct {
	*forms.ExecuteMySQLQueryForm
	C        *gin.Context
	Username string
}

func (s *ExecuteMySQLQueryService) validateStatementSyntax() (*parser.TiStmt, error) {
	// 检查语法是否有效
...
func (s *ExecuteMySQLQueryService) validateStatementNum(stmts []ast.StmtNode) error {
...
func (s *ExecuteMySQLQueryService) validateStatementType(stmt ast.StmtNode) error {
	// 判断语句类型是否被允许
