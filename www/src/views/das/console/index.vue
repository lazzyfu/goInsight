  ...
  <div v-if="data.showbTable" style="margin-top: 15px; overflow-x: auto;">
    <a-tabs default-active-key="1">
      <a-tab-pane key="1" tab="结果集">
        <a-table size="small" class="ant-table-striped" bordered :data-source="data.tableData" :scroll="{ x: '100%' }"
          style="min-width: 100%;">
          <a-table-column v-for="item in data.columns" :key="item" :title="item" :data-index="item" />
        </a-table>
      </a-tab-pane>
    </a-tabs>
  </div>
