<template>
  <div>
    <div class="search-wrapper">
      <a-input-search v-model:value="searchValue" placeholder="输入SQL内容" style="width: 350px" @search="onSearch" />
    </div>
    <div style="margin-top: 14px">
      <a-table size="small" :columns="columns" :row-key="record => record.key" :data-source="data.dataSource"
        @resizeColumn="handleResizeColumn" :pagination="pagination" :loading="data.loading" @change="handleTableChange"
        :scroll="{ x: 1500, }">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'sqltext'">
            <div class="sql-cell">
              <code class="sql-preview">{{ record.sqltext, 50 }}</code>
    ...
    <a-modal v-model:open="modal.open" title="SQL详情" width="55%" :footer="null" @ok="handleOk">
      ...
      <a-card title="重写SQL" style="margin-top: 10px" size="small">
        <highlightjs language="sql" :code="modal.rewrite_sqltext" />
      </a-card>
      <a-card title="参数" style="margin-top: 10px" size="small">
        <highlightjs language="sql" :code="modal.params" />
      </a-card>
    </a-modal>
  </div>
</template>
...
import { CheckSquareOutlined, ClockCircleOutlined, CloseCircleOutlined, CopyOutlined, DatabaseOutlined, EyeOutlined } from '@ant-design/icons-vue';
...
const showSqlDetail = (record) => {
  ...
  modal.rewrite_sqltext = record.rewrite_sqltext
  modal.params = JSON.stringify(record.params);
}
