  ...
  <a-modal :open="props.open" :title="props.title" width="35%" @cancel="handleCancel">
    ...
    <a-form ref="formRef" :model="formData" :rules="rules" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }"
      ...
      <a-form-item label="描述" name="title" has-feedback>
        ...
        <template v-else>
          <a-select v-model:value="formData.title" :options="titleOptions" placeholder="请选择描述"
            :field-names="{ label: 'title', value: 'title' }" allow-clear show-search option-filter-prop="title"
...
const titleOptions = computed(() => props.inspectParams || [])
...
const handleTitleChange = (title) => {
  ...
  const matched = (props.inspectParams || []).find((item) => item.title === title)
  if (matched) {
    formData.value = {
      ...formData.value,
      title: matched.title,
      ...
      type: matched.type,
      value: matched.value,
      _editValue: normalizeEditValue(matched),
    }
  }
}
